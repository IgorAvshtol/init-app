...
export function DropdownMenu({ image }: IDropdownMenu) {
  return (
    <Menu as="div">
      <div className="absolute right-0 h-full flex items-center">
        <Menu.Button>
          <div className="w-28 flex items-center justify-between">
            ...
            <img src={image || avatar} className="h-12" alt="avatar" />
          </div>
        </Menu.Button>
      </div>
      <Menu.Items className="bg-slate-100 rounded-xl border-inherit absolute top-24 right-0">
        <div className="w-32 flex flex-col justify-items-start p-5">
          <Menu.Item>
            {({ active }) => (
              <a className={`${active ? 'underline pb-2' : 'pb-2'}`} href="/account">
                Settings
              </a>
            )}
          </Menu.Item>
          ...
          <Menu.Item>
            {({ active }) => (
              <a className={`${active ? 'underline pt-2' : 'pt-2'}`} href="/">
                Logout
              </a>
            )}
          </Menu.Item>
